...
const CambiarPassword = () => {
  ...
  const handleSubmit = async e => {
    ...
    if( password.pwd_nuevo < 6 ) {
      setAlerta({
        msg: 'El password debe ser mínimo de 6 Caracteres', error: true
      })
      return;
    }
            ...
            <div className='w-full md:w-1/2 bg-white shadow rounded-lg p-5'>
                ...
                <form action="" onSubmit={handleSubmit}>
                    <div className='my-3'>
                        ...
                        <input type="password" name="pwd_actual" id="" className='border bg-gray-50 w-full p-2 rounded-lg' placeholder='Escribe tu password Actual' onChange={ e => setPassword({
                          ...password, [e.target.name]: e.target.value
                        })} />
                    </div>
                    <div className='my-3'>
                        <label htmlFor="" className='uppercase font-bold text-gray-600'>Password Nuevo</label>
                        <input type="password" name="pwd_nuevo" id="" className='border bg-gray-50 w-full p-2 rounded-lg' placeholder='Escribe tu nuevo password' onChange={ e => setPassword({
                          ...password, [e.target.name]: e.target.value
                        })} />
                    </div>
